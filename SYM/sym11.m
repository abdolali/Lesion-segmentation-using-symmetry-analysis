function output = sym11(im1,im2, mode, scaling_factor)
%
...
%%%%%%%       :) C:\Users\meisamh\Documents\MATLAB\1.jpg
...
% COMPUTE KEYPOINTS
% ----------------------------------
[im, keyDescriptor, keyVector] = siftV4_mod(im1);
[im2, keyDescriptor2, keyVector2] = siftV4_mod(im2);
% correct keypoint locatoins to be 1 pixel to the right and lower
keyVector(:,[1,2]) = keyVector(:,[1,2]) + 1;
keyVector2(:,[1,2]) = keyVector2(:,[1,2]) + 1;
% directly modify SIFT feature vector to represent reflected regions
keyDescriptor_m = (keyDescriptor2);
keyVector_m = keyVector2;
...
switch lower(mode)
    case{'rotational','rotational results'}
        ...
        output = [max_rot_centre, sym_strength'];
...
% 
        ...
        %plot([keyVector(:,2), keyVector_m(:,2)]', [keyVector(:,1), keyVector_m(:,1)]', 'w.');
        hold off;
        format_figure(im);
        print('-djpeg','results/butterfly_eg2.jpg');
...
%         quiver(sym_x,sym_y,u,v,0,'g.'); 
        ...
        %             col = [1 1 1];
        %             h1 = quiver(sym_x(i),sym_y(i),u(i),v(i),0,'g.');
        %             set(h1,'LineWidth',axis_width,'Color',phase_weight(i)*col);
        %             h2 = quiver(sym_x(i),sym_y(i),-u(i),-v(i),0,'g.');
...
%         hold off;
%         format_figure(im);
%         print('-djpeg','results/butterfly_eg3.jpg');
...
%                 set(h1,'LineWidth',axis_width,'Color',phase_weight(ind{i}(j))*col);
%                 h2 = quiver(sym_x(ind{i}(j)),sym_y(ind{i}(j)),-u(ind{i}(j)),-v(ind{i}(j)),0,'g.');
%                 set(h2,'LineWidth',axis_width,'Color',phase_weight(ind{i}(j))*col);
% 
...
%                 %                 'g:','Color',phase_weight(ind{i}(j))*col);
%                 %
%                 %             % draw feature points
%                 %             plot([keyVector(match_ind(ind{i}(j)),2), keyVector_m(match_ind_m(ind{i}(j)),2)]', ...
%                 %                 [keyVector(match_ind(ind{i}(j)),1), keyVector_m(match_ind_m(ind{i}(j)),1)]', ...
...
%         hold off;
%         format_figure(im);
%         print('-djpeg','results/butterfly_eg5.jpg');
%         disp(sprintf('%g symmetric features',length(ind{i})));
% 
